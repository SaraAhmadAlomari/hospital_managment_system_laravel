<div>
   ...
   @endif --}}
    ...
    @if ($ServiceSaved)
        <div class="alert alert-info">{{trans('Groups.save')}}</div>
    @endif

    @if ($ServiceUpdated)
        <div class="alert alert-info">{{trans('Groups.edit_s')}}</div>
    @endif
        ...
        <form wire:submit.prevent="saveGroup" autocomplete="off">
            ...
            <div class="card mt-4">
                <div class="card-header">
                    <div class="col-md-12">
                        <button class="btn btn-outline-primary"
                                wire:click.prevent="addService">{{trans('Groups.add_sub_service')}}
                        </button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tr class="table-primary">
                                <th>{{trans('Groups.service_name')}}</th>
                                <th width="200">{{trans('Groups.quantity')}}</th>
                                <th width="200">{{trans('Groups.process')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <select name="GroupsItems[{{$index}}][service_id]"
                                                    ...
                                                    wire:model="GroupsItems.{{$index}}.service_id">
                                                <option value="">{{trans('Groups.choose_prodect')}}</option>
                    ...
                    <br/>
                    <div>
                        <input class="btn btn-outline-success" type="submit" value="{{trans('Groups.confirm')}}">
                    </div>
                </div>
            </div>
